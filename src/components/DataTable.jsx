...
const pageSize = 6; // Số lượng khách hàng hiển thị mỗi trang
...
const DataTable = () => {
  const [customers, setCustomers] = useState([]); // State lưu trữ dữ liệu khách hàng
  const [selectedIds, setSelectedIds] = useState([]); // Lưu trạng thái checkbox đã chọn
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("/data/dataKhachHang.json");
        const data = await response.json();
        setCustomers(data); // Cập nhật dữ liệu vào state

        // Hiển thị dữ liệu trong console
        console.log("Data loaded:", data); // Đây là dòng để hiển thị dữ liệu trong console
      } catch (error) {
  ...
  // Tính tổng số trang và lấy dữ liệu khách hàng trong trang hiện tại
  const totalPages = Math.ceil(customers.length / pageSize);
  const paginatedCustomers = customers.slice((page - 1) * pageSize, page * pageSize);

  const handleSelectAll = (e) => {
    const pageIds = paginatedCustomers.map((o) => o.id);
    if (e.target.checked) {
      setSelectedIds([...new Set([...selectedIds, ...pageIds])]);
    } else {
      setSelectedIds(selectedIds.filter((id) => !pageIds.includes(id)));
  ...
  const handleSelectOne = (id) => {
    setSelectedIds((prev) => {
      const updatedIds = prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id];
      return updatedIds;
    });
  };
          ...
          <tr>
            <th className="p-3">
              <input
                type="checkbox"
                onChange={handleSelectAll}
                checked={paginatedCustomers.length > 0 && paginatedCustomers.every((o) => selectedIds.includes(o.id))}
              />
            </th>
            <th className="p-3">CUSTOMER NAME</th>
            <th className="p-3">COMPANY</th>
        ...
        <tbody>
          ...
          {paginatedCustomers.map((customer) => (
            <tr key={customer.id} className="border-t hover:bg-gray-50 transition">
              ...
              <td className="p-3">{formatCurrency(customer.value)}</td>
              <td className="p-3">{formatDate(customer.date)}</td>
              <td className="p-3 text-center">
                <span
                  className={`px-2 py-1 rounded-full text-xs text-white ${getStatusColor(customer.status)}`}
                >
                  {customer.status}
                </span>
              </td>
        ...
        <div className="flex gap-2">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => (
            <button
              ...
              className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition 
              ${page === pageNum ? "bg-rose-500 text-white font-bold" : "bg-rose-200 text-white hover:bg-rose-400"}`}
            >
              {pageNum}
            </button>
          ))}
